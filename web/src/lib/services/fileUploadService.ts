import { NotFoundError, OperationError, ServiceUnavailableError, ValidationError, PayloadTooLargeError } from "$lib/errors";
...
export class FileUploadService {
    constructor(
        private ctx: AuthenticatedContext
    ) {}
    ...
    async uploadFile(
        ...
        try {
            ...
            if (err instanceof ServiceUnavailableError || err instanceof ValidationError || err instanceof PayloadTooLargeError) {
                throw err;
            }
    ...
    async deleteFile(fileId: string): Promise<void> {
        ...
        if (fileError) {
            if (fileError.code === 'PGRST116') { // Row not found
                throw new NotFoundError('File');
            }
            throw new OperationError(
                `Failed to fetch file: ${fileError.message}`,
                'database.fetch',
                { fileId, errorCode: fileError.code }
            );
        }
