...
export const createOrganization = async (
	...
	if (error) {
		...
		throw new OperationError(`Failed to create organization: ${error.message}`, 'database.insert', {
			errorCode: error.code
		});
	}

	if (!orgId) {
		throw new OperationError('Failed to create organization: No ID returned', 'database.insert');
	}
...
export const updateOrganization = async (
	...
	if (error) {
		throw new OperationError(`Failed to update organization: ${error.message}`, 'database.update', {
...
export const deleteOrganization = async (event: AuthenticatedEvent, organizationId: string) => {
	...
	if (error) {
		throw new OperationError(`Failed to delete organization: ${error.message}`, 'database.delete', {
			orgId: organizationId,
			errorCode: error.code
		});
	}
