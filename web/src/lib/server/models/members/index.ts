...
export const getOrganizationMembers = async (event: AuthenticatedEvent, organizationId: string) => {
	...
	if (error) {
		throw new OperationError(`Failed to fetch members: ${error.message}`, 'database.fetch', {
...
export const getMember = async (
	...
	if (error) {
		...
		throw new OperationError(`Failed to fetch member: ${error.message}`, 'database.fetch', {
			memberId,
			orgId: organizationId,
			errorCode: error.code
		});
...
export const updateMemberRole = async (
	...
	if (error) {
		throw new OperationError(`Failed to update member role: ${error.message}`, 'database.update', {
			memberId,
			orgId: organizationId,
			errorCode: error.code
		});
...
export const removeMember = async (
	...
	if (error) {
		throw new OperationError(`Failed to remove member: ${error.message}`, 'database.delete', {
			memberId,
			orgId: organizationId,
			errorCode: error.code
		});
...
export const addMember = async (
	...
	if (existing) {
		throw new ConflictError('User is already a member of this organization', 'duplicate', existing);
	}
	...
	if (error) {
		if (error.code === '23505') {
			...
			throw new ConflictError('User is already a member of this organization', 'duplicate');
		}
		throw new OperationError(`Failed to add member: ${error.message}`, 'database.insert', {
			userId,
			orgId: organizationId,
			errorCode: error.code
		});
